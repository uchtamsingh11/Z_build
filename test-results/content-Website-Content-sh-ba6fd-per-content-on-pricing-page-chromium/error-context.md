...
Call log:
  - navigating to "http://localhost:3000/pricing", waiting until "load"
    ...
    at C:\Users\singh\Desktop\kk\tests\content.spec.ts:20:16
```
...
>  20 |     await page.goto('/pricing');
  ...
  106 |     if (page.url().includes('/auth/login')) {
  107 |       // We're on login page, just verify the URL
  108 |       expect(page.url()).toContain('/auth/login');
  109 |     } else {
  110 |       // We're on terms page
  111 |       expect(page.url()).toContain('/terms-conditions');
  112 |       
  113 |       // Look for content on the page
  114 |       const paragraphs = await page.locator('p').count();
  115 |       expect(paragraphs).toBeGreaterThanOrEqual(1);
  116 |     }
  117 |   });
  118 | }); 
```